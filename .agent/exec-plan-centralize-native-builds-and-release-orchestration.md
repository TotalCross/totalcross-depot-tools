...
- [x] (2026-07-19T16:22:45Z) Added the central CMake target executor, made the composite action delegate to the existing low-level executor, and migrated zlib's published target wrappers. Validated representative zlib, minizip, SLJIT, and libpng host builds plus a zlib consumer fixture. Evidence: `.agent/evidence/centralize-native-builds-and-release-orchestration.jsonl`.
- [x] (2026-07-19T16:31:26Z) Completed the dependency standard, scaffold, focused header validation, onboarding links, repository skills, and distributable consumer skill. Validated the scaffold lifecycle and a temporary consumer pinned to published `zlib-1.3.1-r3`. Evidence: `.agent/evidence/centralize-native-builds-and-release-orchestration.jsonl`.
- [x] (2026-07-19T19:20:00Z) Added one operation-based workflow entry point for all 15 libraries. CMake libraries use planner-generated matrices and shared target execution; VCRuntime and Skia preserve their specialized build paths. Legacy pairs remain until their later remote-equivalence and obsolete-path gates. Evidence: `.agent/evidence/centralize-native-builds-and-release-orchestration.jsonl`.
...
Milestone 3 made `scripts/build-native-target.sh` the central CMake target path and left `scripts/build-cmake-multi.sh` as its one low-level configure/build/test/install/package implementation. The composite action now calls that lower-level script rather than carrying a second command sequence. Published zlib wrappers delegate only library and target identity; the non-published Android armv7 and macOS x86_64 wrappers were removed. Four representative macOS-arm64 builds preserved their artifact layouts, and the zlib artifact passed a CMake consumer fixture.

Milestone 4 confirmed and completed the repository onboarding surface. The already-tracked dependency standard, consumption guide, scaffold, repository skills, AGENTS guidance, and plan guidance form one coherent contract. The scaffold now reads central target policy and supports dry-run; header checks can focus on explicit or staged paths. The distributable consumer skill is included and was exercised with a pinned published zlib checkout.
