...
- [ ] Consolidate Apple builds and XCFramework packaging where measurement shows that runner reuse is preferable to parallel execution (completed for SQLite, qrcode/qrcodegen, axTLS, and mbedTLS; remote macOS validation and the remaining image/compression-library workflows are still pending).
