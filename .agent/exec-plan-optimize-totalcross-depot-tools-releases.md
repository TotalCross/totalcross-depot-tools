...
- [ ] Consolidate Apple builds and XCFramework packaging where measurement shows that runner reuse is preferable to parallel execution (completed for SQLite, qrcode/qrcodegen, axTLS, mbedTLS, zlib, zlib-ng, libjpeg, and libjpeg-turbo; remote macOS validation plus minizip, minizip-ng, libpng, and Skia are still pending).
...
The first implementation pass added semantic dependency routing, Android toolchain verification, minimal image definitions, baseline metrics collection, shared workflow primitives, dry-run stack orchestration, scoped Skia cache/diagnostic changes, metadata-job removal from individual release wrappers, and Apple consolidation for SQLite plus the shared qrcode/qrcodegen workflow. A zlib GitHub Actions dry-run on 2026-07-17 succeeded across its full platform matrix and skipped all state-changing publication steps. Local YAML, shell, Python, and router tests passed. No cold/warm comparison or Docker image smoke run was measured because this workstation has no Docker. Broader Apple consolidation and ARMv7 equivalence remain incomplete and must not be represented as delivered.
