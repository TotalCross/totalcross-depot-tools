...
- [ ] Consolidate Apple builds and XCFramework packaging where measurement shows that runner reuse is preferable to parallel execution (completed for SQLite and the shared qrcode/qrcodegen workflow; remote macOS validation and the remaining small-library workflows are still pending).
