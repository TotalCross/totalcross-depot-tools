...
function(tcvm_auto_fetch_libpng)
  ...
  elseif(NOT DEFINED LIBPNG_RELEASE_TAG)
    if(COMMAND tcvm_get_dependency_release)
      tcvm_get_dependency_release(libpng LIBPNG_RELEASE_TAG "libpng-1.6.48-r3")
    else()
      set(LIBPNG_RELEASE_TAG "libpng-1.6.48-r3")
    endif()
  endif()
  ...
  elseif(APPLE AND (CMAKE_SYSTEM_NAME STREQUAL "iOS" OR CMAKE_OSX_SYSROOT MATCHES "[Ii][Pp]hone"))
    if(CMAKE_OSX_SYSROOT MATCHES "[Ii][Pp]hone[Ss]imulator")
      set(TCVM_LIBPNG_PLATFORM "ios-simulator")
    else()
    ...
    if(TCVM_LIBPNG_ARCH)
      ...
      if(TCVM_LIBPNG_ARCH_LEN GREATER 1)
        message(FATAL_ERROR "libpng auto-fetch expects a single iOS architecture, got: ${TCVM_LIBPNG_ARCH}")
      endif()
      list(GET TCVM_LIBPNG_ARCH 0 TCVM_LIBPNG_ARCH)
    else()
      set(TCVM_LIBPNG_ARCH "arm64")
    endif()
    if(TCVM_LIBPNG_ARCH STREQUAL "aarch64")
      set(TCVM_LIBPNG_ARCH "arm64")
    endif()
  elseif(WIN32)
    set(TCVM_LIBPNG_PLATFORM "windows")
    set(TCVM_LIBPNG_WINDOWS_PLATFORM "${CMAKE_VS_PLATFORM_NAME}")
    if(NOT TCVM_LIBPNG_WINDOWS_PLATFORM)
      set(TCVM_LIBPNG_WINDOWS_PLATFORM "${CMAKE_GENERATOR_PLATFORM}")
    endif()
    string(TOLOWER "${TCVM_LIBPNG_WINDOWS_PLATFORM}" TCVM_LIBPNG_WINDOWS_PLATFORM_LOWER)
    if(TCVM_LIBPNG_WINDOWS_PLATFORM_LOWER STREQUAL "win32")
      set(TCVM_LIBPNG_ARCH "x86")
    elseif(TCVM_LIBPNG_WINDOWS_PLATFORM_LOWER STREQUAL "x64")
      set(TCVM_LIBPNG_ARCH "x64")
    elseif(TCVM_LIBPNG_WINDOWS_PLATFORM_LOWER STREQUAL "arm64")
      set(TCVM_LIBPNG_ARCH "arm64")
