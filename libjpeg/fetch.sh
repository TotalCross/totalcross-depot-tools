...
Options:
  ...
  --release-tag TAG        GitHub release tag, default: libjpeg-10
  ...
  --github-token-env NAME  Environment variable containing a GitHub token,
                           default: LIBJPEG_GITHUB_TOKEN, then GITHUB_TOKEN
  --dest DIR               Destination root, default: libjpeg/local
EOF
}
...
if [[ ! "${github_repo}" =~ ^[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+$ ]]; then
  echo "Invalid libjpeg GitHub repository value. Expected OWNER/REPO." >&2
  exit 2
fi

if [[ "${release_tag}" == *"{"* || "${release_tag}" == *"}"* || "${release_tag}" == *" "* ]]; then
  echo "Invalid libjpeg release tag value." >&2
  exit 2
fi
...
asset_name="libjpeg-${platform}-${arch}.tar.gz"
...
requirements=(
  ...
  'include/jmorecfg.h'
  'include/jpeglib.h'
  'lib/libjpeg.a|lib/jpeg.lib|lib/libjpeg.lib'
)
if tc_artifact_marker_matches "$dest" "libjpeg" "$github_repo" "$release_tag" "$asset_name" "" "${requirements[@]}"; then
  echo "Reusing libjpeg ${platform}/${arch} from ${dest}"
  exit 0
fi


if ! tc_github_release_download "${github_repo}" "${release_tag}" "${asset_name}" "${archive}" \
  "${github_token_env}" LIBJPEG_GITHUB_TOKEN; then
  echo "Unable to download a libjpeg artifact for ${platform}/${arch}" >&2
  exit 1
fi
...
if ! find "${artifact_root}/lib" -type f \( -name "libjpeg.a" -o -name "jpeg.lib" -o -name "libjpeg.lib" \) | grep -q .; then
  echo "Unable to find libjpeg static library under ${artifact_root}/lib" >&2
  exit 1
fi
...
tc_artifact_replace_tree "${artifact_root}" "$dest" "libjpeg" "$github_repo" "$release_tag" "$asset_name" \
  "$TC_GITHUB_RELEASE_DOWNLOADED_SHA256" "${requirements[@]}"

echo "Installed libjpeg ${platform}/${arch} into ${dest}"
