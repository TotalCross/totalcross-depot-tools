...
if tc_artifact_marker_matches "$dest" "minizip" "$github_repo" "$release_tag" "$asset_name" "" "${requirements[@]}"; then
  echo "Reusing minizip ${platform}/${arch} from ${dest}"
  exit 0
fi


if ! tc_github_release_download "${github_repo}" "${release_tag}" "${asset_name}" "${archive}" \
  "${github_token_env}" MINIZIP_GITHUB_TOKEN; then
...
tc_artifact_replace_tree "${artifact_root}" "$dest" "minizip" "$github_repo" "$release_tag" "$asset_name" \
