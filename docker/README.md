...
workflows. Version `v2.0.4` is the minimal native-CMake generation: Ubuntu
...
`skia-linux-amd64` is separate because Skia needs Python, ccache, rsync,
Fontconfig, EGL, and GLES2 development headers, and the AArch64 cross compiler. The ARMv7
compatibility image also includes Python, Fontconfig, EGL, and GLES2 headers for Skia. GN is
obtained deterministically from Skia's pinned `depot_tools` checkout rather
than an Ubuntu package. Build jobs must not install packages at runtime.
