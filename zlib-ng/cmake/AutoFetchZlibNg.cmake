...
function(tcvm_auto_fetch_zlibng)
  ...
  elseif(APPLE AND (CMAKE_SYSTEM_NAME STREQUAL "iOS" OR CMAKE_OSX_SYSROOT MATCHES "iphone"))
    if(CMAKE_OSX_SYSROOT MATCHES "iphonesimulator")
      set(TCVM_ZLIB_NG_PLATFORM "ios-simulator")
    else()
    ...
    if(TCVM_ZLIB_NG_ARCH)
      ...
      if(TCVM_ZLIB_NG_ARCH_LEN GREATER 1)
        message(FATAL_ERROR "zlib-ng auto-fetch expects a single iOS architecture, got: ${TCVM_ZLIB_NG_ARCH}")
      endif()
      list(GET TCVM_ZLIB_NG_ARCH 0 TCVM_ZLIB_NG_ARCH)
    else()
      set(TCVM_ZLIB_NG_ARCH "arm64")
    endif()
    if(TCVM_ZLIB_NG_ARCH STREQUAL "aarch64")
      set(TCVM_ZLIB_NG_ARCH "arm64")
    endif()
  elseif(WIN32)
    set(TCVM_ZLIB_NG_PLATFORM "windows")
    set(TCVM_ZLIB_NG_WINDOWS_PLATFORM "${CMAKE_VS_PLATFORM_NAME}")
    if(NOT TCVM_ZLIB_NG_WINDOWS_PLATFORM)
      set(TCVM_ZLIB_NG_WINDOWS_PLATFORM "${CMAKE_GENERATOR_PLATFORM}")
    endif()
    string(TOLOWER "${TCVM_ZLIB_NG_WINDOWS_PLATFORM}" TCVM_ZLIB_NG_WINDOWS_PLATFORM_LOWER)
    if(TCVM_ZLIB_NG_WINDOWS_PLATFORM_LOWER STREQUAL "win32")
      set(TCVM_ZLIB_NG_ARCH "x86")
    elseif(TCVM_ZLIB_NG_WINDOWS_PLATFORM_LOWER STREQUAL "x64")
      set(TCVM_ZLIB_NG_ARCH "x64")
    elseif(TCVM_ZLIB_NG_WINDOWS_PLATFORM_LOWER STREQUAL "arm64")
      set(TCVM_ZLIB_NG_ARCH "arm64")
