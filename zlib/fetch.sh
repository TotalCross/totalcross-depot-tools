...
Options:
  ...
  --dest DIR               Destination root, default: zlib/local
EOF
}
...
if [[ ! "${github_repo}" =~ ^[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+$ ]]; then
  echo "Invalid zlib GitHub repository value. Expected OWNER/REPO." >&2
  exit 2
fi

if [[ "${release_tag}" == *"{"* || "${release_tag}" == *"}"* || "${release_tag}" == *" "* ]]; then
  echo "Invalid zlib release tag value." >&2
  exit 2
fi
...
elif [ -n "${ZLIB_GITHUB_TOKEN:-}" ]; then
  github_token="${ZLIB_GITHUB_TOKEN}"
...
download_release_asset() {
  ...
  echo "Downloading zlib artifact ${candidate} from ${github_repo}@${release_tag}"
  ...
  echo "Direct zlib artifact download failed; trying GitHub release asset API"
...
if ! download_release_asset "${asset_name}" "${archive}"; then
  echo "Unable to download a zlib artifact for ${platform}/${arch}" >&2
  exit 1
fi
...
if ! find "${artifact_root}/lib" -type f \( -name "libz.a" -o -name "z.lib" -o -name "zlib.lib" -o -name "zlibstatic.lib" \) | grep -q .; then
  echo "Unable to find zlib static library under ${artifact_root}/lib" >&2
  exit 1
fi
