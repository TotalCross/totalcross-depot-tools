...
Options:
  ...
  --release-tag TAG        GitHub release tag, default: minizip-ng-4.2.2
  ...
  --github-token-env NAME  Environment variable containing a GitHub token,
                           default: MINIZIP_NG_GITHUB_TOKEN, then GITHUB_TOKEN
  --dest DIR               Destination root, default: minizip-ng/local
EOF
}
...
if [[ ! "${github_repo}" =~ ^[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+$ ]]; then
  echo "Invalid minizip-ng GitHub repository value. Expected OWNER/REPO." >&2
  exit 2
fi

if [[ "${release_tag}" == *"{"* || "${release_tag}" == *"}"* || "${release_tag}" == *" "* ]]; then
  echo "Invalid minizip-ng release tag value." >&2
  exit 2
fi
...
asset_name="minizip-ng-${platform}-${arch}.tar.gz"
...
requirements=(
  ...
  'include/unzip.h'
  'include/zip.h'
  'lib/libminizip*.a|lib/minizip*.a|lib/minizip*.lib|lib/libminizip*.lib'
)
if tc_artifact_marker_matches "$dest" "minizip-ng" "$github_repo" "$release_tag" "$asset_name" "" "${requirements[@]}"; then
  echo "Reusing minizip-ng ${platform}/${arch} from ${dest}"
  exit 0
fi


if ! tc_github_release_download "${github_repo}" "${release_tag}" "${asset_name}" "${archive}" \
  "${github_token_env}" MINIZIP_NG_GITHUB_TOKEN; then
  echo "Unable to download a minizip-ng artifact for ${platform}/${arch}" >&2
  exit 1
fi
...
include_header="$(find "${tmp_dir}" -path "*/include/mz.h" -type f | head -n 1)"
if [ -z "${include_header}" ]; then
  echo "Unable to find include/mz.h in ${asset_name}" >&2
  exit 1
fi
...
if ! find "${artifact_root}/lib" -type f \( -name "libminizip*.a" -o -name "minizip*.a" -o -name "minizip*.lib" -o -name "libminizip*.lib" \) | grep -q .; then
  echo "Unable to find minizip-ng static library under ${artifact_root}/lib" >&2
  exit 1
fi
...
tc_artifact_replace_tree "${artifact_root}" "$dest" "minizip-ng" "$github_repo" "$release_tag" "$asset_name" \
  "$TC_GITHUB_RELEASE_DOWNLOADED_SHA256" "${requirements[@]}"

echo "Installed minizip-ng ${platform}/${arch} into ${dest}"
